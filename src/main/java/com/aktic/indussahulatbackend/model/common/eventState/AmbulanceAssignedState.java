...
public class AmbulanceAssignedState implements EventState{
    @Override
    public EventStatus getStatus() {
        return EventStatus.AMBULANCE_ASSIGNED;
    }

    @Override
    public void next(IncidentEvent event, EventState nextState) {
        if(!(event.getState() instanceof AmbulanceAssignedState)) {
            throw new IllegalStateException("Event is not in the correct state to move to the next state.");
        }
        if(!(nextState instanceof DriverAcceptedState) && !(nextState instanceof QuestionnaireFilledState)) {
